...
| `restmd` | `crates/restmd` | active | `restmd` binary (clap): `restmd [DIR]` opens the TUI, `restmd init [DIR]` scaffolds a `.restmd` dir, `restmd check [PATHS]` validates files, `restmd format [PATHS] [--check]` canonicalizes them; `run` planned. |
